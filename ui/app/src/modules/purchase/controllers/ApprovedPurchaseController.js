...
angular.module('purchase').controller('ApprovedPurchaseController', ['$scope', '$rootScope', '$stateParams', 'config', '$modal', 'dialogs', 'DateTimeService', 'toaster', 'User','ngTableParams', 'PurchaseOrder','$q','$state',
    function ($scope, $rootScope, $stateParams, config, $modal, dialogs, DateTimeService, toaster, User, ngTableParams, PurchaseOrder, $q, $state) {
        ...
        $scope.purchaseApprovedTable = new ngTableParams({
            page: 1,            // show first page
            count: 5           // count per page
        }, {
            total: 0, // length of data
            getData: function($defer, params) {
                var purchases = PurchaseOrder.$search({keyword: $scope.search.purchaseOrder, page: params.page(), sort: params.orderBy(), status: $scope.status.approved});
                $scope.total = PurchaseOrder.count({keyword: $scope.search.purchaseOrder, status: $scope.status.approved});
